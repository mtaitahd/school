<?php
/**
 * Phase 5 — Migration Fix: Upsert + Validate NUM-02 Activities
 *
 * Safe to re-run (idempotent).
 * 1. Creates topic + lessons if missing (calls v10a)
 * 2. Upserts all 40 activities (calls v10b)
 * 3. Validates JSON, engine whitelist
 */
require_once __DIR__ . '/../php/db_connection.php';

echo "=== Phase 5 Fix: NUM-02 Upsert + Validate ===\n\n";

/* 1. Ensure topic + lessons exist */
require __DIR__ . '/run_migration_v10a.php';
echo "\n";

/* 1b. Get correct module_id */
$mod = $database->fetchOne("SELECT module_id FROM modules WHERE module_name = 'Number Zero' LIMIT 1");
$module_id = $mod ? (int)$mod['module_id'] : 14;
echo "module_id = $module_id\n\n";

/* 2. Load activity definitions */
require __DIR__ . '/run_migration_v10b.php';

/* 3. Build lesson_code → lesson_id map */
$L = [];
$rows = $database->fetchAll("SELECT lesson_id, lesson_code FROM lessons WHERE lesson_code LIKE 'NUM-02-%'");
foreach ($rows as $r) { $L[$r['lesson_code']] = (int)$r['lesson_id']; }
echo "Lessons found: " . count($L) . "\n";
if (count($L) < 4) { echo "ERROR: expected 4 NUM-02 lessons\n"; exit(1); }
...
/* 5. Validate */
$check = $database->fetchAll("SELECT a.activity_id, a.activity_name, a.activity_data FROM activities a JOIN lessons l ON a.lesson_id = l.lesson_id WHERE l.lesson_code LIKE 'NUM-02-%' ORDER BY l.order_index, a.step_order");
echo "\nTotal NUM-02 activities in DB: " . count($check) . "\n";
...
$perLesson = $database->fetchAll("SELECT l.lesson_code, l.lesson_name, COUNT(*) as cnt FROM activities a JOIN lessons l ON a.lesson_id = l.lesson_id WHERE l.lesson_code LIKE 'NUM-02-%' GROUP BY l.lesson_code ORDER BY l.order_index");
